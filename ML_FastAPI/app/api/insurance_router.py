...
def predict_insurance(payload: InsuranceInput):
    """
    Predict the insurance premium for a person based on demographic and health features.
    """

    try:
        # Construct a DataFrame with the correct column names
        df = pd.DataFrame([{
            "age": payload.age,
            "sex": payload.sex,
            "bmi": payload.bmi,
        ...
        # Apply preprocessing (encoder/scaler pipeline)
        try:
            X_scaled = insurance_scaler.transform(df)
        except Exception as e:
            print("⚠️ Preprocessor skipped:", e)
            X_scaled = df
